...
/*温控仪设备是宇电温控，物模型暂时只定义一个PV,一个SV,两个温度的上下限为+-100摄氏度
对于属性上报的报文，格式为      PV16+SV16+状态+MV+校验码16     
校验码确定后，先从地址开始解析
读/写报文，温控设备地址设定为01H,地址是两个相同字节，值为01H+80H
举例，读给定值报文，0X81 0X81 0X52 0 0 0 0X53
功能码  52H 为读指令  43H为写指令
参数代码  00H 为给定值 
数据解析  低八位+高八位
...
{						//传入参数为消息体，publish报文的头部和主题已经被处理，在此处报文要重新打包成AlinkJson报文上传到IOT平台
	...
	*/
    ...
    var wkAddress=wkChecksum(uploadTempMessage);           //报文校验码验证,验证温控报文还是传感器报文并计算设备地址

    if(wkAddress!=0)
    {
        return wkMessageUpload(uploadTempMessage,wkAddress);         //处理温控报文
    }
    else
    {
        if(CheckTHSensorCRC(uploadTempMessage))
        {
            return(THSensorMessageUpload(uploadTempMessage));   //处理传感器报文
        }
        else
        {
...
function wkMessageUpload(wkUpMessage,wkUpAddress)
{
	...
	var wkUpDataView = new DataView(wkUploadTempMessage.buffer, 0);		//dataview类 可以用多种类型值来读取报文数据，buffer属性访问只能读不能修改
    var jsonMap = new Object();				//object 对象
    ...
    jsonMap['method'] = ALINK_PROP_REPORT_METHOD;	//转换为ALink JSON格式，此处为上报到IOT平台的属性上报topic。数组编号内容为键，赋值内容为值
    jsonMap['version'] = '1.0';						//ALink JSON格式，协议版本号固定字段。
    jsonMap['id'] = ''+wkUpAddress ;		  //ALink JSON格式，返回设备地址
    var params = new Object();							//也可以用params ={}  ;物模型的参数，这里要和物模型定义的参数匹配
    var temp=new Uint8Array(wkUploadTempMessage.buffer,0,wkUploadTempMessage.length-2);				//除校验码外，数据掉头，此处wkUploadTempMessage.buffer数据会被修改

    temp.reverse();                         //把数据报文掉头，再从后面开始取数据，以实现高位在前，地位在后的问题
			
    var wkUploadtempDataView=new DataView(temp.buffer);

    params['SetTemperature'] = (wkUploadtempDataView.getInt16(4))/dPt;        //温控设备属性的设置值
    params['CurrentTemperature'] = (wkUploadtempDataView.getInt16(6))/dPt;        //温控设备属性的过程值，即实际温度,注意小数点位置
...
function wkChecksum(checkTempMessage)
{
    ...
    if(checkTempMessage.length%2!=0)           //温控仪的数据是偶数位，偶数位数据不会出现dataview offset溢出
    {
        return 0;                       //非偶数位一定不是温控仪
    }
    ...
    {
        reverseMessage[i]=checkTempMessage[i];                   //引用类型都会指向同一个堆，所以需要拷贝数据再处理
    }

    reverseMessage.reverse();                       //报文掉头
    var CheckSumDataView=new DataView(reverseMessage.buffer);

	var temp1=CheckSumDataView.getUint16(0);			//报文自带的校验码
	



	for(var i=2;i<reverseMessage.length;i++)            //计算校验值
	{
		temp2=temp2+CheckSumDataView.getUint16(i);
        i++;
	}

    temp2=temp2&0xffff;                             //校验和超0XFFFF溢出后js会自动调整为32位数据，所以要把高位数据去掉

    for(var i=1;i<WK_DEVICES_SUM+1;i++)           //计算后的校验值+地址，验证温控设备地址，地址值一定大于0
    {
        if((temp2+i)==temp1)
        {
            return i;                           //返回设备地址
