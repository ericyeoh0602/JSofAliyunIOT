...
/*自动滚压线2号线现场数据采集，现场HMI采用的是16字节的数据存储
...
function messageUploadAnalysis(UpMessage)
{
    ...
    var bytesCounter=messageAnalysisView.getUint8(2);   //通过判断返回字节数来判断报文内容
    var tempArray=new Uint8Array(4);        //存储4字节浮点格式数


    if (bytesCounter==20)  //设备状态和异常信息
    {
        var temp=messageAnalysisView.getUint8(6);
        if((temp&0x01)==0x01)           //手动或自动控制位
        {
            params['StateAndException:Manual']=1;
            params['StateAndException:Auto']=0;
        }
        
        if((temp&0x02)==0x02)           
        {
            params['StateAndException:Manual']=0;
            params['StateAndException:Auto']=1;
        }

        if((temp&0x04)==0x04)           //设备停止位
        {
            params['StateAndException:Stop']=1;
        }
        else
        {
            params['StateAndException:Stop']=0;
        }
        
    }
    else if(bytesCounter==24)           //产量、1号成型时间等
    {
        temp=messageAnalysisView.getUint32(3);     //总产量
        params['Molding:TotalProduct']=temp;
        
        tempArray[0]=messageAnalysisView.getUint8(15);   //1号成型时间
        tempArray[1]=messageAnalysisView.getUint8(16);
        tempArray[2]=messageAnalysisView.getUint8(17);
        tempArray[3]=messageAnalysisView.getUint8(18);
        params['Molding:MoldingTime1']=hexToSingle(tempArray);   //转浮点数
        tempArray[0]=messageAnalysisView.getUint8(19);   //周期
        tempArray[1]=messageAnalysisView.getUint8(20);
        tempArray[2]=messageAnalysisView.getUint8(21);
        tempArray[3]=messageAnalysisView.getUint8(22);
        params['Molding:Period']=hexToSingle(tempArray);   //转浮点数


    }
    else if(bytesCounter==4)       //2号成型时间
    {
        tempArray[0]=messageAnalysisView.getUint8(3);   //2号成型时间
        tempArray[1]=messageAnalysisView.getUint8(4);
        tempArray[2]=messageAnalysisView.getUint8(5);
        tempArray[3]=messageAnalysisView.getUint8(6);
        params['Molding:MoldingTime2']=hexToSingle(tempArray);   //转浮点数
    }
    else if(bytesCounter==44)      //温度相关数据
    {
        tempArray[0]=messageAnalysisView.getUint8(3);   //1号成型温度
        tempArray[1]=messageAnalysisView.getUint8(4);
        tempArray[2]=messageAnalysisView.getUint8(5);
        tempArray[3]=messageAnalysisView.getUint8(6);
        params['Temperature:SVMolding1']=hexToSingle(tempArray);   //转浮点数
        tempArray[0]=messageAnalysisView.getUint8(7);   //2号成型温度
        tempArray[1]=messageAnalysisView.getUint8(8);
        tempArray[2]=messageAnalysisView.getUint8(9);
        tempArray[3]=messageAnalysisView.getUint8(10);
        params['Temperature:SVMolding2']=hexToSingle(tempArray);   //转浮点
        tempArray[0]=messageAnalysisView.getUint8(11);   //1号风机温度
        tempArray[1]=messageAnalysisView.getUint8(12);
        tempArray[2]=messageAnalysisView.getUint8(13);
        tempArray[3]=messageAnalysisView.getUint8(14);
        params['Temperature:SVFan1']=hexToSingle(tempArray);   //转浮点数
        tempArray[0]=messageAnalysisView.getUint8(15);   //2号风机温度
        tempArray[1]=messageAnalysisView.getUint8(16);
        tempArray[2]=messageAnalysisView.getUint8(17);
        tempArray[3]=messageAnalysisView.getUint8(18);
        params['Temperature:SVFan2']=hexToSingle(tempArray);   //转浮点


        tempArray[0]=messageAnalysisView.getUint8(31);   //1号成型温度PV
        tempArray[1]=messageAnalysisView.getUint8(32);
        tempArray[2]=messageAnalysisView.getUint8(33);
        tempArray[3]=messageAnalysisView.getUint8(34);
        params['Temperature:PVMolding1']=hexToSingle(tempArray);   //转浮点数
        tempArray[0]=messageAnalysisView.getUint8(35);   //2号成型温度PV
        tempArray[1]=messageAnalysisView.getUint8(36);
        tempArray[2]=messageAnalysisView.getUint8(37);
        tempArray[3]=messageAnalysisView.getUint8(38);
        params['Temperature:PVMolding2']=hexToSingle(tempArray);   //转浮点
        tempArray[0]=messageAnalysisView.getUint8(39);   //1号风机温度PV
        tempArray[1]=messageAnalysisView.getUint8(40);
        tempArray[2]=messageAnalysisView.getUint8(41);
        tempArray[3]=messageAnalysisView.getUint8(42);
        params['Temperature:PVFan1']=hexToSingle(tempArray);   //转浮点数
        tempArray[0]=messageAnalysisView.getUint8(43);   //2号风机温度PV
        tempArray[1]=messageAnalysisView.getUint8(44);
        tempArray[2]=messageAnalysisView.getUint8(45);
        tempArray[3]=messageAnalysisView.getUint8(46);
        params['Temperature:PVFan2']=hexToSingle(tempArray);   //转浮点

    }
    else if(bytesCounter==114)     //1号滚头设定数据
    {
        tempArray[0]=messageAnalysisView.getUint8(7);   //1号Y轴成型位置
        tempArray[1]=messageAnalysisView.getUint8(8);   
        tempArray[2]=messageAnalysisView.getUint8(9);
        tempArray[3]=messageAnalysisView.getUint8(10);
        params['Setting1:YMoldingPosition']=hexToSingle(tempArray);   //转浮点数
      

    }
    else if(bytesCounter==116)     //2号滚头设定数据,为区分两个滚头，所以多读了一个16位字节，解析时要去掉
    {
        tempArray[0]=messageAnalysisView.getUint8(9);   //2号Y轴成型位置
        tempArray[1]=messageAnalysisView.getUint8(10);   
        tempArray[2]=messageAnalysisView.getUint8(11);
        tempArray[3]=messageAnalysisView.getUint8(12);
        params['Setting2:YMoldingPosition']=hexToSingle(tempArray);   //转浮点数
    }
...
//0x70696e67
var test=new Uint8Array([0x01,0x03,0x04,0x40,0xda,0xe1,0x48,0x86,0x6e]);
